...
function show_license() {
	...
	if(VIEW_OFF) {
		...
		$hrefurl = empty($info['url']) ? 'https://gitee.com/YHCMS/YHCMSX/releases' : $info['url'];
...
<meta content="YanHe Inc." name="Copyright" />
</head>
<div class="container{$nostep}">
	<div class="header">
		<h1>$titlehtml</h1>
		<span>YHCMS! $version $install_lang $reldisp</span>
EOT;
		...
		<script type="text/javascript">
			...
			function request_do_db_init() {
				ajax.get('index.php?<?= http_build_query(array('method' => 'do_db_init', 'allinfo' => $allinfo)) ?>', function(data) {
					if(data.indexOf('YHCMS! Database Error') !== -1 || data.indexOf('YHCMS! System Error') !== -1 || data.indexOf('Fatal error') !== -1) {
			...
			function request_do_initsys() {
				...
				if(resultDiv.innerHTML.indexOf('<?= lang('initdbdataresult_succ') ?>') !== -1) {
					...
					ajax.get('../misc.php?mod=initsys', function(callback, status) {
						if(status >= 400 || callback.indexOf('Access Denied') !== -1 || callback.indexOf('YHCMS! Database Error') !== -1 || callback.indexOf('YHCMS! System Error') !== -1 || callback.indexOf('Fatal error') !== -1) {
							var p = document.createElement('p');
							p.className = 'red';
							if(status >= 400) {
...
function install_uc_server() {
	...
	$db->query("INSERT INTO {$uctablepre}applications SET name='YHCMS! Board', url='$appurl', ip='$ucip', authkey='$appauthkey', synlogin='1', charset='$uccharset', dbcharset='$ucdbcharset', type='DISCUZX', recvnote='1', tagtemplates=''");
	$appid = $db->insert_id();
